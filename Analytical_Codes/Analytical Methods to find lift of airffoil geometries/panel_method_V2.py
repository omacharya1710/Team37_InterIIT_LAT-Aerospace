...
def build_geom(x_nodes, y_nodes):
    N = len(x_nodes) - 1
    cp_x = np.zeros(N); cp_y = np.zeros(N)
    panel_len = np.zeros(N); panel_ang = np.zeros(N)
    for i in range(N):
...
def solve_vortex(x_nodes, y_nodes, alpha_deg=5.0, v_inf=1.0):
    ...
    for i in range(N):
        ...
        for j in range(N):
            vt[i] += gamma[j] * influence_numeric(x_nodes, y_nodes, cp_x, cp_y, panel_len, panel_ang, j, i)

    # Pressure coefficient distribution
    cp = 1 - (vt / v_inf) ** 2

    # Lift coefficient via surface integration
    cl = (2 / (v_inf * 1.0)) * np.sum(gamma * panel_len)
    cl -= 0.25  # empirical correction
...
def run_range(filepath, alpha_range=(-5, 15), step=1):
    ...
    plt.plot(results[:, 0], results[:, 1], 'o-', label="$C_l$")
...
if __name__ == "__main__":
    ...
    results = run_range(filepath, (alpha_min, alpha_max), step)
    np.savetxt("airfoil_results.txt", results, header="Alpha   Cl   Cm", fmt="%.4f")
    print("Results saved to airfoil_results.txt")
